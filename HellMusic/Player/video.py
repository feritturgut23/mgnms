...
async def generate_cover(thumbnail, title, userid, ctitle):
    ...
    draw.text((120, 595), f"Oynatılıyor: {ctitle[:20]}...", fill="white", stroke_width = 1, stroke_fill="white" ,font=font)
...
async def vplay(c: Client, m: Message):
    ...
    if not a.can_manage_voice_chats:
        await m.reply_text(
        "💡 Beni kullanmak için aşağıdaki izni aşağıda verin:"
        + "\n\n» ❌ __Manage video chat__\n\nOnce done, try again.")
        return
    ...
    if replied:
        if replied.video or replied.document:
            loser = await replied.reply("📥 **video indirme...**")
            ...
            if len(m.command) < 2:
                ...
                if pq == "720" or "480" or "360":
                    ...
                    Q = 720
                    await loser.edit(
                        "» __sadece 720, 480, 360 izin verilir__ \n💡 **şimdi 720p'de video akışı**"
                    )
            try:
                if replied.video:
            ...
            if chat_id in QUEUE:
                ...
                await loser.edit("🔄 **Sesli sohbete katılıyorum...**")
                ...
                await m.reply_photo(
                    photo="https://i.ibb.co/Qkz78hx/images-1.jpg",
                    reply_markup=InlineKeyboardMarkup(buttons),
                    caption=f"🗂 **İsmi:** [{songname}]({link}) | `video`\n💭 **Sohbet:** `{chat_id}`\n🧸 **Talep eden:** {requester}",
                )
        else:
            if len(m.command) < 2:
                await m.reply_photo(
                ...
                if search == 0:
                    ...
                    if abhi == 0:
                        await loser.edit(f"❌ yt-dl algılanan sorunlar\n\n» `{ytlink}`")
                    else:
                        if chat_id in QUEUE:
                            pos = add_to_queue(
                            ...
                            await m.reply_photo(
                                photo=image,
                                reply_markup=InlineKeyboardMarkup(buttons),
                                caption=f"💡 **Kuyruğa eklenen parça »** `{pos}`\n\n🗂 **İsmi:** [{songname}]({url}) | `video`\n⏱ **Süre:** `{duration}`\n🧸 **Talep eden:** {requester}",
                            )
                        else:
                            try:
                                await loser.edit("🔄 **Sesli sohbete katılıyor...**")
                                await call_py.join_group_call(
                                    chat_id,
                                    AudioVideoPiped(
                                        ytlink,
                                ...
                                await m.reply_photo(
                                    photo="https://i.ibb.co/Qkz78hx/images-1.jpg",
                                    reply_markup=InlineKeyboardMarkup(buttons),
                                    caption=f"🗂 **İsmi:** [{songname}]({url}) | `video`\n⏱ **Süre:** `{duration}`\n🧸 **Talep eden:** {requester}",
                                )
        ...
        if len(m.command) < 2:
            await m.reply_photo(
                     photo=f"{IMG_6}",
                    caption="💬**Kullanım: /oynat ile başlatınız veya /vplay Video oynatmak için**"
                    ,
                      reply_markup=InlineKeyboardMarkup(
                    [
                        [
            ...
            amaze = HighQualityVideo()
            if search == 0:
                await loser.edit("❌ **no bulunan sonuçlar.**")
            else:
                songname = search[0]
                title = search[0]
                url = search[1]
...
async def vstream(c: Client, m: Message):
    ...
    if a.status != "administrator":
        await m.reply_text(
            f"💡 Beni kullanmak için, bir **Yönetici** aşağıdakiler **permissions**:\n\n» ❌ __Delete messages__\n» ❌ __Invite users__\n» ❌ __Manage video chat__\n\nOnce done, type /reload"
        )
        return
    if not a.can_manage_voice_chats:
        await m.reply_text(
    ...
    if len(m.command) < 2:
        await m.reply("» bana canlı bağlantı / m3u8 url / youtube bağlantısı akışı verin.")
    else:
        if len(m.command) == 2:
            link = m.text.split(None, 1)[1]
            Q = 720
            loser = await c.send_message(chat_id, "🔄 **Video akışı işleniyor...**")
            ...
            if chat_id in QUEUE:
                ...
                try:
                    ...
                    await m.reply_photo(
                        photo=f"{IMG_2}",
                        reply_markup=InlineKeyboardMarkup(buttons),
                        caption=f"💡 **[Video Canlı]({link}) akış başlatıldı.**\n\n💭 **Sohbet:** `{chat_id}`\n🧸 **Talep eden:** {requester}",
                    )
                except Exception as ep:
                    await loser.delete()
                    await m.reply_text(f"🚫 error: `{ep}`")
