...
async def stream_end_handler(_, u: Update):
    if isinstance(u, StreamAudioEnded):
        ...
        if op==1:
           await bot.send_message(chat_id, "✅ **userbot'un görüntülü sohbet bağlantısı kesildi.**")
        elif op==2:
           await bot.send_message(chat_id, "❌ **bir hata oluştu**\n\n» **Temizleme** __Sıra__ **ve görüntülü sohbeti bırakma.**")
        else:
         await bot.send_message(chat_id, f"💡 **Bir sonraki parça akışı**\n\n☑️ **İsm:** [{op[0]}]({op[1]}) | `{op[2]}`\n💭 **Sohbet:** `{chat_id}`", disable_web_page_preview=True, reply_markup=keyboard)
    else:
        pass
